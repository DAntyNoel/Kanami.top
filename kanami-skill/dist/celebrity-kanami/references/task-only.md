...
- 单纯角色闲聊、正史人物问答、关系场景和声线展示应交还统一 `SKILL.md`，由其路由到包内的 `persona-only.md` 契约。
...
合格输出必须让用户看见真实结果、验证依据与剩余不确定性，而不是只看见角色化承诺。任务完成后由统一 `SKILL.md` 合并必要的 persona 表现；若请求转回纯角色对话，则交还统一入口并按需加载 `persona-only.md`。
